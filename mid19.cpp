...
int main(int argc, char *argv[])
{
    ...
    // If we are testing, run the testing framework.
#ifdef TESTING
    if (argc <= 1) {
        printf("\nUsage: unittest <tolerance>\n\n");
        return 0;
    }

    double tolerance = stof(argv[1]);
    int count = 0;
    ...
    printf("     init x    init y   exp x    act x    exp y    act y      length     actual\n");
    printf("     -------- -------  -------- -------- -------- ---------  ---------- -------");
    int i=0;
    Vec v = { test_data[i*5+0], test_data[i*5+1] };
    Flt test_x, test_y, length, exp_x, exp_y;


    do {
        Flt ret = vec_length(v);
        test_x = test_data[i*5+0];
        test_y = test_data[i*5+1];
        exp_x = test_data[i*5+2];
        exp_y = test_data[i*5+3];
        length = test_data[i*5+4];
        //printf("\n%3i  %8.5lf  %8.5lf  %10.5lf %10.5lf", i+1, test_x, test_y, length, ret);
        printf("\n%3i  %8.5lf %8.5lf %8.5lf %8.5lf %8.5lf %8.5lf %10.5lf %10.5lf", i+1, test_x, test_y, exp_x, v[0], exp_y, v[1], length, ret);
        if (abs(ret - length) > tolerance) {
            printf("\x1b[31m <-- error\x1b[0m");
            count++;
        }
        // Test to see if values match expected xy normalization
        if (abs(v[0] - exp_x) > tolerance) {
            printf("\x1b[31m <-- error, x result doesn't match expected result\x1b[0m");
            count++;
        }
        if (abs(v[1] - exp_y) > tolerance) {
            printf("\x1b[31m <-- error, y result doesn't match expected result \x1b[0m");
            count++; 
        }
    ...
    // If we are not testing, run regular framework
#else
    printf("Calculation of vector lengths:      \n");
    printf("                                    \n");
    printf("     x         y         length     \n");
